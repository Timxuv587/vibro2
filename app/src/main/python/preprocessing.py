...
def nothing_pca():
    df = pd.read_csv('../test/noobject-acc.csv')
    df = pca_sensor(df)
...
def kiwi_pca():
    df = pd.read_csv('../test/kiwi123gbound-acc.csv')
    df = pca_sensor(df)
...
def orange1_pca():
    df = pd.read_csv('../test/orange192gbound-acc.csv')
    df = pca_sensor(df)
...
def orange2_pca():
    df = pd.read_csv('../test/orange227gbound-acc.csv')
    df = pca_sensor(df)
    df = df[(df['time_tick']<35) & (df['time_tick']>15)]
    t = df['time_tick'].values
    acc = df['acc_pca'].values
