...
def fruit(path,delay,starttime):
    df = pd.read_csv(path)
    df = df[df['time_tick']>(starttime+delay)]
    t = df['time_tick'].values
    t = (t - t[0])/1000
...
if __name__ == '__main__':
    ...
    os.chdir('../data/7.2_nexus2_rotate/apple/')
    weights = []
    intensity = []
    ...
    #  Plot raw 
    # ======================================================================
    fig, _ = plt.subplots(4, 6)
    # fig.suptitle('Sharing x per column, y per row')

    #     0: Fruit name, 1: ID, 2: weight, 3: start time, 4: cycle, 5: ratio, 6: repeat

    for cnt, ax in enumerate(fig.get_axes()):
        title_prefix = 'APPLE_' + str(cnt+1) + '_'
        for current_file in glob.glob("*.csv"):
            if current_file.startswith(title_prefix):
                param = current_file.split('_')
                t, x, y, z = fruit(path=current_file, delay=int(param[4])*(1-float(param[5])), starttime=int(param[3]))
                ax.plot(x)
                # ax.set_ylim(0, 20)
                ax.set_title(param[2])
                # z -= np.mean(z)
                # intense = np.mean(np.abs(z))
                # intensity.append(intense)
                print("\nfruit:{}, ID:{}, weight(g):{} starttime:{}".format(param[0],param[1],param[2], param[3]))
    ...
    #     if param[0] == 'KIWI':
    #         try:
    #             t, x, y, z = fruit(current_file,int(param[4])*(1-float(param[5])),int(param[3]))
    ...
    #         except:
    #             print('Data import error', param)

    # plt.scatter(weights, intensity)
    # plt.show()
    ...
    # for cnt, ax in enumerate(fig.get_axes()):
    #     title_prefix = 'KIWI_' + str(cnt+1) + '_'
    #     for current_file in glob.glob("*.csv"):
    #         if current_file.startswith(title_prefix):
    #             param = current_file.split('_')
    #             t, x, y, z = fruit(current_file,int(param[4])*(1-float(param[5])),int(param[3]))
    ...
    #             NFFT = 1024  # the length of the windowing segments
    #             Fs = 26.24  # the sampling frequency
    ...
    #             # intense = np.mean(np.abs(z))
    #             # intensity.append(intense)
    #             print("{} {} {} {}".format(param[0],param[1],param[2], param[3]))
